# -*- coding: utf-8 -*-
"""
Created on Fri Aug  8 09:28:53 2025

@author: PAUDUPE
"""

import pandas as pd
from pathlib import Path

# Definieer paden
base_path = Path("C://github//gebiedsniveaus//")
input_path = base_path / "kerntabellen//statsec_elzantw.xlsx"
output_path1 = base_path / "data_voor_swing//gebiedsdefinities//elzantw.xlsx"
output_path2 = base_path / "data_voor_swing//gebiedsdefinities//elzantw_dummy.xlsx"

# Laad Excel-bestand
col_names = ['statsec','ELZantw', 'Name ELZantw']
dtypes = {'statsec':'string','elzantw':'Int64','Name ELZantw':'string'} 
# Mocht het niveau letters hebben (bv. zzzz), dan string

df = pd.read_excel(
    input_path, 
    sheet_name="statsec_elzantw",
    names=col_names,
    dtype=dtypes
)

# Groepeer (AGGREGATE-equivalent)
df_grouped = df.groupby(['ELZantw', 'Name ELZantw']).size().reset_index(name='aantal_gebieden')

# Datasetbewerking
df_grouped = df_grouped.drop(columns=['aantal_gebieden'])
df_grouped = df_grouped.rename(columns={
    'ELZantw': 'geoitem code',
    'Name ELZantw': 'short name'
})
df_grouped['name'] = df_grouped['short name']


# Voeg volgnummer toe
df_grouped = df_grouped.reset_index(drop=True)
df_grouped['sequence nr'] = df_grouped.index + 1
print(df_grouped.dtypes)

# Volgnummer als integer met nul decimalen (in principe niet nodig)
df_grouped['sequence nr'] = df_grouped['sequence nr'].astype('int64')

# Houd alleen de relevante kolommen
output_df = df_grouped[['sequence nr', 'geoitem code', 'short name', 'name']]

# Opslaan naar Excel
for path in [output_path1, output_path2]:
    output_df.to_excel(
        path,
        index=False, 
        engine='openpyxl'
)
