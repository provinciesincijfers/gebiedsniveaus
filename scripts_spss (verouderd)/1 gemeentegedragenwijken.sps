...
AGGREGATE
  /OUTFILE='testuniek'
  /BREAK=gemeentecode Naam_wijk Gebiedscode_pinc
  /N_BREAK=N.


DATASET ACTIVATE testuniek.
* Identify Duplicate Cases.
SORT CASES BY Gemeentecode(A) NAAM_wijk(A).
MATCH FILES
  /FILE=*
  /BY Gemeentecode NAAM_wijk
  /FIRST=PrimaryFirst
  /LAST=PrimaryLast.
DO IF (PrimaryFirst).
...
* Identify Duplicate Cases.
SORT CASES BY Gebiedscode_pinc(A).
MATCH FILES
  /FILE=*
  /BY Gebiedscode_pinc
...
match files
/file=*
/keep=codsec Naam_wijk Gebiedscode_pinc provinciegemaakt mengvorm_nafusies.
...
if Gebiedscode_pinc="" Gebiedscode_pinc=dena_nis7.
if Naam_wijk="" Naam_wijk=dena_nis7.
EXECUTE.
...
dataset close nis7.
rename variables Naam_wijk=ggw7_naam.
rename variables Gebiedscode_pinc=ggw7.
...
* dat MOET steeds 1 of 2 3 of 4 zijn, anders is er iets mis in de data! 
